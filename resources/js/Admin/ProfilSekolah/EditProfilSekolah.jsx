...
export default function EditProfilSekolah({ profil, onClose }) {
  // useForm untuk handle data form + validasi + request
  ...
  // isi form otomatis dengan data profil yang diterima dari props
  useEffect(() => {
    if (profil) {
      setData({
  ...
  const submit = (e) => {
    ...
    post(`/admin/update/profil/sekolah/${profil.id}`, {
      onSuccess: () => {
        ...
        onClose();   // tutup modal/form setelah berhasil update
      },
    });
  };

  return (
    // form utama untuk edit data profil sekolah
